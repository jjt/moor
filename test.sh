...
if [[ "${MOOR_SKIP_INTERACTIVE_TESTS}" != "1" && -t 1 ]]; then
  ...
  echo Test auto quitting with --quit-if-one-screen=screen...
  echo "  (success)" | ./moor --quit-if-one-screen=screen

  echo Test auto quitting on width-only fit...
  echo "  (success)" | ./moor --quit-if-one-screen=width

  echo Test auto quitting on height-only fit...
  echo "  (success)" | ./moor --quit-if-one-screen=height

  echo Test auto quitting with space-separated axis...
  echo "  (success)" | ./moor --quit-if-one-screen height
fi
